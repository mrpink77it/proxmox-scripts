...
select_gpu() {
    ...
    GPU_PCI=$(whiptail --title "Selezione GPU (v1.0.5)" \
...
main_menu() {
    ...
    while true; do
        ...
        menu_items+=("3" "Crea VM Cloud-Init (Wizard Avanzato)")
        ...
        CHOICE=$(whiptail --title "Proxmox 9 GPU Manager (v1.0.5)" \
...
dump_vbios() {
    ...
    if cat /sys/bus/pci/devices/$GPU_PCI/rom > "$ROM_PATH" 2>/dev/null; then
        ...
        if [ "$ROM_SIZE" -gt 50000 ]; then
            ...
            whiptail --title "DUMP COMPLETATO" --msgbox "Il vBIOS è stato estratto con successo!\n\nPercorso: $ROM_PATH\nDimensioni: $ROM_SIZE bytes\n\nNome file associabile: $ROM_NAME" 12 75
        else
            rm -f "$ROM_PATH"
            whiptail --title "ERRORE DUMP (Shadowed ROM)" --msgbox "L'estrazione ha prodotto un file vuoto o corrotto.\n\nLa ROM primaria è ombreggiata. Scarica il vBIOS da TechPowerUp." 10 75
        fi
    else
        echo 0 > /sys/bus/pci/devices/$GPU_PCI/rom || true
        whiptail --title "ERRORE I/O" --msgbox "Impossibile leggere il file ROM. La periferica è bloccata a livello hardware." 10 70
    fi
...
bind_vfio() {
    ...
    whiptail --title "VFIO Attivo" --msgbox "La GPU è associata a vfio-pci. Puoi avviare la VM in sicurezza." 10 70
}
...
create_test_vm() {
    ...
    # 3. Richiesta CPU
    VM_CORES=$(whiptail --title "Assegnazione CPU" --inputbox "Quanti vCPU core vuoi assegnare alla VM?\n\nIl tuo host Proxmox ha $HOST_CORES core logici totali." 12 60 "4" 3>&1 1>&2 2>&3)
    [ -z "$VM_CORES" ] && return

    # 4. Richiesta RAM
    VM_RAM=$(whiptail --title "Assegnazione RAM" --inputbox "Quanta RAM (in MB) vuoi assegnare alla VM?\n\nIl tuo host Proxmox ha $HOST_RAM_MB MB di RAM totali." 12 60 "8192" 3>&1 1>&2 2>&3)
    [ -z "$VM_RAM" ] && return

    # 5. Scelta Storage Dinamica
    local ST_OPTIONS=()
    ...
    if [ ${#ST_OPTIONS[@]} -eq 0 ]; then
        whiptail --title "Errore Storage" --msgbox "Nessuno storage abilitato per le immagini VM (Disk image) trovato su questo nodo." 10 60
        return
    fi
    ...
    # 6. Richiesta Spazio Disco
    VM_DISK=$(whiptail --title "Assegnazione Disco" --inputbox "Quanto spazio su disco (in GB) vuoi assegnare?\n(Verrà aggiunto il suffisso G automaticamente)" 12 60 "40" 3>&1 1>&2 2>&3)
    [ -z "$VM_DISK" ] && return
    ...
    echo -e "${GREEN}Scaricamento immagine in corso (se non presente)...${NC}"
    wget -nc -q --show-progress "$IMG_URL" || true

    # Comandi Proxmox QM usando le variabili dell'utente
    ...
    whiptail --title "Creazione VM Completata" --msgbox "La VM $VMID è pronta con le seguenti specifiche:\n- CPU: $VM_CORES Cores\n- RAM: $VM_RAM MB\n- Disco: ${VM_DISK}G su $STORAGE\n- Passthrough: $PT_OPTS\n\nImposta la password in Cloud-Init (GUI) e usa il menu VFIO (Opzione 4) prima di accenderla." 15 75
}
